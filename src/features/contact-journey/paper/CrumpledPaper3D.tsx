...
function PaperMesh({ transitioning = false }: PaperMeshProps) {
  ...
  const {
    ...
    const calibTranslate = centroid0.clone().multiplyScalar(-calibScale);
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < vertCount; i++) {
      const bi = i * 3;
      let dx = smoothed[bi] - cx, dy = smoothed[bi + 1] - cy, dz = smoothed[bi + 2] - cz;
