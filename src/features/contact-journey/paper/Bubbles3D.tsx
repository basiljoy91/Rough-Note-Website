...
function BubbleGroup() {
  ...
  // Generate random data once to avoid continuous reconstruction
  const bubbles = useMemo(() => {
    const arr = [];
    for (let i = 0; i < BUBBLE_COUNT; i++) {
      // Irregular, scattered positions, avoiding perfectly blocking the center
      const angle = Math.random() * Math.PI * 2;
      const radius = 1.5 + Math.random() * 3.5; // push away from exact center
      
      const x = Math.cos(angle) * radius;
      const y = (Math.random() - 0.5) * 5;
      
      // Depth variation for layered 3D effect
      const z = (Math.random() - 0.5) * 3;
      
      // Mixture of very small to small/medium
      const scale = 0.04 + Math.random() * 0.12; 
      
      const speed = 0.15 + Math.random() * 0.25;
      const offset = Math.random() * Math.PI * 2;
      
      arr.push({ x, y, z, scale, speed, offset });
    }
    return arr;
  }, []);
